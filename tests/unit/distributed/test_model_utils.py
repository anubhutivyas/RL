...
class ModelUtilsTestActor:
    ...
    def test_packed_sequences_equivalence(self):
        ...
        unpacked_target_ids = torch.arange(batch_size * seq_len).reshape(batch_size, seq_len).to("cuda")
        ...
        with_cp_logprobs = from_parallel_logits_to_logprobs(
            _get_tokens_on_this_cp_rank(unpacked_logits, cp_rank, self.cp_size, seq_dim=1),
            unpacked_target_ids,
            vocab_start_index,
            vocab_end_index,
            tp_group,
        ...
        packed_logits = _get_tokens_on_this_cp_rank(unpacked_logits, cp_rank, self.cp_size, seq_dim=1).reshape(1, -1, vocab_part_size)
...
def register_model_utils_test_actor():
    ...
    if MODEL_UTILS_TEST_ACTOR_FQN in ACTOR_ENVIRONMENT_REGISTRY:
        if original_registry_value is None:
            ...
            ACTOR_ENVIRONMENT_REGISTRY[MODEL_UTILS_TEST_ACTOR_FQN] = original_registry_value
...
@pytest.mark.unit
@pytest.mark.parametrize("tp_cp_config", [
    (2, 1),  # TP=2, CP=1
    (1, 2),  # TP=1, CP=2
])
def test_from_parallel_logits_to_logprobs_packed_sequences(
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() < world_size:
        pytest.skip(f"Not enough GPUs available. Need {world_size}, got {torch.cuda.device_count()}")
    ...
    try:
        ...
        sharding = NamedSharding(layout=np.arange(world_size).reshape(tp_size, cp_size), names=["tp", "cp"])
...
class AllGatherCPTestActor:
    ...
    def test_allgather_cp_tensor(self):
        ...
        if original_seq_len % (2 * self.cp_size) != 0:
            original_seq_len = (original_seq_len // (2 * self.cp_size) + 1) * (2 * self.cp_size)
        
        # Create original tensor (same on all ranks for testing)
        torch.manual_seed(42)  # Same seed for reproducibility
        original_tensor = torch.arange(batch_size * original_seq_len * hidden_size, dtype=torch.float32).reshape(batch_size, original_seq_len, hidden_size).to("cuda")
        ...
        gathered_tensor = allgather_cp_sharded_tensor(sharded_tensor, cp_group, seq_dim=1)
        
        # Verify shapes match
        if gathered_tensor.shape != original_tensor.shape:
            return {"success": False, "error": f"Shape mismatch: expected {original_tensor.shape}, got {gathered_tensor.shape}"}
        ...
        torch.testing.assert_close(grad_sharded, _get_tokens_on_this_cp_rank(2 * original_tensor, rank, self.cp_size, seq_dim=1), rtol=1e-5, atol=1e-5)
        torch.testing.assert_close(_get_tokens_on_this_cp_rank(grad, (rank + 1) % self.cp_size, self.cp_size, seq_dim=1), torch.zeros_like(sharded_tensor), rtol=1e-5, atol=1e-5)
        ...
        gathered_tensor_dim0 = allgather_cp_sharded_tensor(sharded_tensor_dim0, cp_group, seq_dim=0)
        
        # Verify shapes and content match
        if gathered_tensor_dim0.shape != original_tensor_dim0.shape:
            return {"success": False, "error": f"Shape mismatch for seq_dim=0: expected {original_tensor_dim0.shape}, got {gathered_tensor_dim0.shape}"}
        ...
        sharded_2d = _get_tokens_on_this_cp_rank(original_2d, rank, self.cp_size, seq_dim=0)
...
def register_allgather_cp_test_actor():
    ...
    original_registry_value = ACTOR_ENVIRONMENT_REGISTRY.get(ALLGATHER_CP_TEST_ACTOR_FQN)
    ...
    if ALLGATHER_CP_TEST_ACTOR_FQN in ACTOR_ENVIRONMENT_REGISTRY:
        if original_registry_value is None:
            ...
            ACTOR_ENVIRONMENT_REGISTRY[ALLGATHER_CP_TEST_ACTOR_FQN] = original_registry_value
