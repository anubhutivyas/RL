...
def grpo_train(
    ...
    while step < max_num_steps:
        ...
        with timer.time("total_step_time"):
            ...
            max_iterations = master_config["grpo"]["dapo_max_iterations"]
            iteration_count = 0
            ...
            # Calculate metrics on final filtered samples
            with timer.time("reward_calculation"):
                rewards = repeated_batch["total_reward"]
                
                # Get advantages from the filtered batch
                advantages = torch.cat([msg["advantages"][:1] for msg_log in repeated_batch["message_log"] for msg in msg_log if msg["role"] == "assistant"])
                
                percent_valid_advantages = (advantages.count_nonzero() / advantages.numel())
                percent_zero_advantages = 1 - percent_valid_advantages
                
                advantages_min, advantages_mean, advantages_max = (
                    advantages.min(), advantages.mean(), advantages.max()
                )
                reward_min, reward_mean, reward_max = (
                ...
                for message_log in repeated_batch["message_log"]:
                    for message in message_log:
                        if message["role"] == "assistant":
                            total_gen_tokens += len(message["token_ids"])
                
                rollout_metrics["mean_gen_tokens_per_sample"] = total_gen_tokens / len(repeated_batch["message_log"])
            ...
            print(flat_messages["advantages"], flat_messages["advantages"].shape, "flat_messages advantages")
